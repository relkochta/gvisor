...
func TestKafka(t *testing.T) {
	...
	compatibility.Poll(ctx, t, "kafka broker to be ready", readyTimeout, pollInterval, func() error {
		if out, err := run(bin + "kafka-topics.sh --bootstrap-server " + bootstrap + " --list"); err != nil {
	...
	// Create a topic.
	if out, err := run(bin + "kafka-topics.sh --bootstrap-server " + bootstrap +
		" --create --topic " + topic + " --partitions 1 --replication-factor 1"); err != nil {
		t.Fatalf("create topic: %v\n%s", err, out)
	}

	// Produce a single message.
	if out, err := run("echo " + want + " | " + bin + "kafka-console-producer.sh --bootstrap-server " +
		bootstrap + " --topic " + topic); err != nil {
		t.Fatalf("produce: %v\n%s", err, out)
	}

	// Consume it back.
	out, err := run(bin + "kafka-console-consumer.sh --bootstrap-server " + bootstrap +
