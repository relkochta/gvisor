...
func TestWordPress(t *testing.T) {
	...
	// The REST API reads the default seed post back out of MySQL. Use the
	// query-string route (/?rest_route=), which works regardless of the permalink
	// structure - a fresh install only enables pretty /wp-json/ permalinks when it
	// detects mod_rewrite, so /wp-json/ is not reliably available.
